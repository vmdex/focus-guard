...
Окрема тема для наступного обговорення: як зберігати daily progress.

Потрібно буде вирішити:

- де зберігати daily progress локально;
- як відрізняти focus-only progress від progress разом із counted breaks;
- коли саме комітити прогрес: completed, reset, pause-before-sleep, app shutdown;
- як робити daily reset;
- чи зберігати коротку history по днях уже в прототипі.

Поточний UI-віджет daily progress можна використовувати як прототип відображення, але persistence і day history мають бути окремим наступним кроком.

## Interruption and recovery topics

Окрема тема для наступного обговорення: логіка переривань, щоб не втрачати дані.

Потрібно продумати сценарії:

- PC переходить у sleep;
- користувач блокує Windows;
- застосунок отримує system shutdown/restart;
- процес різко завершується;
- застосунок закривають через `X`, але таймер має продовжуватися або коректно зберегти стан.

Ключове питання: в які моменти застосунок має швидко зберігати active timer state і daily progress, щоб після повернення або повторного запуску можна було коректно відновити або завершити сесію.
